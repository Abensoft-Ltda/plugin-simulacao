  ...
  return (
    <div className="mt-4 rounded-lg bg-gray-800/90 p-3 shadow-lg backdrop-blur-sm flex-grow overflow-y-scroll font-mono">
      <div className="flex items-center justify-between pb-2 border-b border-gray-700/50 mb-2">
        ...
        <button
          onClick={onClear}
          className="cursor-pointer rounded bg-gray-700/50 pl-2 pr-2 pt-0.5 pb-0.5 text-xs text-gray-400 transition-all hover:bg-gray-600/50 hover:text-white"
        >
          Clear
        </button>
      </div>
      ...
      {logs.length === 0 ? (
        <div className="text-center text-sm text-gray-500 italic pt-2 pb-2">
          No logs yet.
        </div>
      ) : (
        <ul className="m-0 list-none p-0 space-y-1">
          {logs.map((log, i) => (
            <li
              key={i}
              className="text-xs text-gray-400 whitespace-pre-wrap break-words"
            >
