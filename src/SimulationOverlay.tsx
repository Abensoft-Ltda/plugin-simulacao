...
const PrimaryButton = forwardRef<HTMLButtonElement, PrimaryButtonProps>(
  (
    {
      importantPadding = "12px 32px",
      importantRadius = "12px",
      style,
      children,
      ...rest
    },
...
export const SimulationOverlay: React.FC<SimulationOverlayProps> = ({
  ...
  isComplete = false,
  isFailure = false,
  failureMessage = "A simulação falhou", 
  children,
}) => {
  ...
  // Mostrar animação de sucesso ou falha
  useEffect(() => {
    if (isFailure) {
      setShowFailure(true);
      ...
      <>
        ...
        >
          <div className="text-center">
            <div className="w-20 h-20 bg-red-500 rounded-full flex items-center justify-center mx-auto mb-4 transform transition-all duration-700 ease-out scale-110 animate-pulse">
              <svg
            ...
            <p
              ...
              style={{ color: "white" }}
              data-font="success-message" // Reutilizando o estilo da fonte
            >
              {failureMessage}
            </p>
            <PrimaryButton
              onClick={() => setIsWatching(true)}
              className="simulation-overlay-primary-btn pl-8 pr-8 pt-3 pb-3 bg-red-500 hover:bg-red-600 border-none rounded-lg text-base font-bold cursor-pointer shadow-lg transition-all duration-300 ease-in-out hover:scale-105"
