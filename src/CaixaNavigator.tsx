...
export const CaixaNavigator: React.FC<{ data: Record<string, any> }> = ({
  ...
  function checkForErrorDialog(): boolean {
    ...
    const errorDialog = document.querySelector(
      "#ui-id-34.ui-dialog-content.ui-widget-content"
    );
    ...
    if (errorDialog) {
      ...
      if (errorText?.includes("Campo obrigatório não informado")) {
        registerLog(
          'Error dialog contains "Campo obrigatório não informado" - refreshing page'
        );
    ...
    for (const dialog of allDialogs) {
      ...
      if (dialogText?.includes("Campo obrigatório não informado")) {
        registerLog(
          `Found error dialog with class "${dialog.className}" containing error text - refreshing page`
        );
        window.location.reload();
        return true;
      }
  ...
  async function simulateNaturalInput(
    ...
    for (let i = 0; i < retries; i++) {
      try {
        ...
        registerLog(
          ` Simulating input for ${selector} with value "${value}" (Attempt ${
            i + 1
          })`
        );
        ...
        registerLog(
          ` Simulating "Enter" key press (1st time) for ${selector}.`
        );
        ...
        registerLog(
          ` Simulating "Enter" key press (2nd time) for ${selector}.`
        );
        targetInput.dispatchEvent(enterEventDown);
        targetInput.dispatchEvent(enterEventPress);
        targetInput.dispatchEvent(enterEventUp);
        ...
        if (finalValue.trim() !== "") {
          registerLog(
            ` Success: ${selector} has a value "${finalValue}". Assuming success.`
          );
          printLogs();
          return;
        } else {
          throw new Error(
            `Failed to verify value for ${selector}. Field is empty.`
          );
        }
      } catch (error: any) {
        registerLog(
          ` Attempt ${i + 1} failed for ${selector}: ${error.message}`
        );
        printLogs();
        if (i === retries - 1) {
          throw new Error(
            ` All ${retries} attempts failed for selector: ${selector}`
          );
        }
  ...
  async function simulateAutocomplete(
    ...
    try {
      ...
      if (cached && cached.optionsHash === optionsHash) {
        ...
        registerLog(
          ` Using cached lookup table for ${selector} (${
        ...
        cityLookup = {};
        registerLog(
          ` Building new lookup table for ${selector} (${options.length} options)...`
        );
        ...
        registerLog(
          ` Cached lookup table with ${Object.keys(cityLookup).length} cities`
        );
      }
      ...
      if (selectedOption && selectedOption.value === optionValue) {
        registerLog(
          ` Success: Set ${selector} to "${selectedOption.text}" (value: ${optionValue})`
        );
        printLogs();
      } else {
  ...
  React.useEffect(() => {
    ...
    const runAutomation = async () => {
      ...
      if (checkForErrorDialog()) {
        registerLog(
          " Error dialog detected during initial check - page will refresh"
        );
        return;
      }
      ...
      if (checkForErrorDialog()) {
        registerLog(
          " Error dialog detected after page load - page will refresh"
        );
        return;
      }
      ...
      if (checkForErrorDialog()) {
        registerLog(
          " Error dialog detected after filling first page - page will refresh"
        );
        return;
      }
      ...
      if (!nextButton1) {
        registerLog(
          ' Automation failed: "Next" button not found on first page.'
        );
        return;
      }
      ...
      if (checkForErrorDialog()) {
        registerLog(
          " Error dialog detected after clicking next button - page will refresh"
        );
        return;
      }
      ...
      if (checkForErrorDialog()) {
        registerLog(
          " Error dialog detected after filling second page - page will refresh"
        );
        return;
      }
      ...
      if (!nextButton2) {
        registerLog(
          ' Automation failed: "Next" button not found on second page.'
        );
        return;
      }
      ...
      registerLog(
        " Automation sequence for first part completed. Second part will be handled by CaixaNavigatorSecondStep.tsx"
      );
      printLogs();
    };
  ...
  async function fillFirstPage(fields: Record<string, any>) {
    ...
    if (fields.categoria_imovel) {
      registerLog(
        ` Filling #grupoTipoFinanciamento with specific type: ${fields.categoria_imovel}`
      );
      await simulateNaturalInput(
